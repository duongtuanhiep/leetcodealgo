...
// Question 2179: https://leetcode.com/problems/count-good-triplets-in-an-array
...
// Question 315: https://leetcode.com/problems/count-of-smaller-numbers-after-self/
func countSmaller(nums []int) []int {
	res := make([]int, len(nums))
	fwTree := fenwickTree(1e4*2 + 1) // Creating a fw trees
	for i := len(nums) - 1; i >= 0; i-- {
		res[i] = fwTree.query(nums[i] + 1e4 - 1) // Query for all elems less than nums[i]
		fwTree.update(nums[i]+1e4, 1)            // add current elems
	}
